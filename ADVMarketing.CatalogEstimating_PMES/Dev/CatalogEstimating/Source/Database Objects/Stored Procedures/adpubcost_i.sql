...
INSERT INTO #tempHostPackagePolybagMap(EST_Component_ID, EST_Package_ID, EST_Polybag_ID, Quantity, DistributionPct,
	PackageWeight, PolybagWeight)
SELECT tc.EST_Component_ID, p.EST_Package_ID, pb.EST_Polybag_ID, pb.Quantity, ppbm.DistributionPct,
	dbo.PackageWeight(p.EST_Package_ID), dbo.PolybagWeight(pb.EST_Polybag_ID)
FROM #tempComponent tc JOIN EST_PackageComponentMapping pcm ON tc.EST_Component_ID = pcm.EST_Component_ID
...
UPDATE #tempHostPackagePolybagMap
	SET
		PolybagCost = dbo.PolybagCost(EST_Polybag_ID) * ISNULL(DistributionPct, PackageWeight / PolybagWeight),
		InkjetCost = dbo.PolybagInkjetCost(EST_Polybag_ID) * ISNULL(DistributionPct, PackageWeight / PolybagWeight),
		InkjetMakereadyCost = dbo.PolybagInkjetMakereadyCost(EST_Polybag_ID) * ISNULL(DistributionPct, PackageWeight / PolybagWeight),
		MailHouseTimeValueSlipsCost = dbo.PolybagMailHouseTimeValueSlipsCost(EST_Polybag_ID) * ISNULL(DistributionPct, PackageWeight / PolybagWeight),
		MailListCost = dbo.PolybagMailListCost(EST_Polybag_ID) * ISNULL(DistributionPct, PackageWeight / PolybagWeight),
		MailHouseAdminFee = dbo.PolybagMailHouseAdminFee(EST_Polybag_ID) * ISNULL(DistributionPct, PackageWeight / PolybagWeight),
		MailHouseGlueTackCost = dbo.PolybagMailHouseGlueTackCost(EST_Polybag_ID) * ISNULL(DistributionPct, PackageWeight / PolybagWeight),
		MailHouseTabbingCost = dbo.PolybagMailHouseTabbingCost(EST_Polybag_ID) * ISNULL(DistributionPct, PackageWeight / PolybagWeight),
		MailHouseLetterInsertionCost = dbo.PolybagMailHouseLetterInsertionCost(EST_Polybag_ID) * ISNULL(DistributionPct, PackageWeight / PolybagWeight),
		MailHouseOtherDirectMailHandlingCost = dbo.PolybagMailHouseOtherDirectMailHandlingCost(EST_Polybag_ID) * ISNULL(DistributionPct, PackageWeight / PolybagWeight)
...
UPDATE tc
	SET MailListCost = BlendedMailListCPM * SoloQuantity / 1000
						+ ISNULL((SELECT SUM(pb.MailListCost) FROM #tempHostPackagePolybagMap pb WHERE tc.EST_Component_ID = pb.EST_Component_ID), 0)
FROM #tempComponentPubRateMap tc
WHERE tc.EST_ComponentType_ID = 1
...
update tc
	set PolyBagCost = ISNULL((SELECT SUM(pb.PolybagCost) FROM #tempHostPackagePolybagMap pb WHERE tc.EST_Component_ID = pb.EST_Component_ID), 0)
FROM #tempComponentPubRateMap tc
where tc.EST_ComponentType_ID = 1
...
update tc
	set PolyBagTotal = isnull(PolyBagCost, 0) + isnull(OnsertCost, 0),
		...
		InkJetCost =
			case EST_ComponentType_ID
				when 1 then (isnull(SoloQuantity, 0) / cast(1000 as decimal) * InkJetRate) + ISNULL((SELECT SUM(InkjetCost) FROM #tempHostPackagePolybagMap pb WHERE tc.EST_Component_ID = pb.EST_Component_ID), 0)
			end,
		InkJetMakeReadyCost =
			case EST_ComponentType_ID
				when 1 then
...
update tc
	set CornerGuardCost = TotalEstimateWeight / cast(2000 as decimal) * CornerGuardRate,
		...
		TimeValueSlipsCost = TimeValueSlipsCPM * SoloQuantity / cast(1000 as decimal)
			+ ISNULL((SELECT SUM(MailHouseTimeValueSlipsCost) FROM #tempHostPackagePolybagMap pb WHERE tc.EST_Component_ID = pb.EST_Component_ID), 0),
		MailHouseAdminFee =
			case
