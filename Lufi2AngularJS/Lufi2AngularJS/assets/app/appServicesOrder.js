...
appServices.factory('order', ['$http', '$cacheFactory', 'serviceArrayFix', 'itemSearch', '$q', 'loggerService', 'sendSMTPErrorEmail', 'btProp', 'POSService', '$rootScope',
    function ($http, $cacheFactory, serviceArrayFix, $itemSearch, $q, $loggerService, $sendSMTPErrorEmail, $btProp, $POSService, $rootScope) {
    ...
    var getOrderList = function (searchParam, success, error) {
        ...
        //var url = 'http://10.131.135.75:7080/Order/GetOrderListJSON';
    ...
    var addSolarData = function (orderline) {
        return $itemSearch.searchItemBySKU(orderline.Item._ItemID).then(function (data) {
            ...
            if (angular.isString(orderline.btDisplay.colorimageid) && (orderline.btDisplay.colorimageid.trim() !== "")) {
                orderline.btDisplay.defaultImageUrl = serviceURL.toString() + "/image/BonTon/" + orderline.btDisplay.colorimageid.trim();
            } else if (angular.isDefined(orderline.btDisplay.imageid) && (orderline.btDisplay.imageid.toString().trim() !== "")) {
    ...
    var getOrderDetail = function (orderNumberParam, success, error) {
        ...
        //var url = "http://10.131.135.75:7080/Order/GetOrderDetailJSON";
    ...
    $rootScope.$on('$stateChangeSuccess', function (event, toState, toParams, fromState, fromParams) {
        ...
        var cachedResults = getPreviousSearchResult();

        if (angular.isArray(cachedResults) && (cachedResults.length > 0) &&  !(stateName === 'orderDetail' || stateName === 'orderSearch')) {
            //if back on Home page, delete previous Search Params
            cache.put("orderSearchResult", []);
            cache.put("orderSearchParam", []);
        }
    ...
    var canCancelOrder = function (orderDetail) {
        ...
        if (angular.isDefined(orderDetail) && angular.isString(orderDetail._DocumentType) && angular.isString(orderDetail._DraftOrderFlag) && angular.isDefined(orderDetail.OrderLines) && angular.isArray(orderDetail.OrderLines.OrderLine)) {

            if (orderDetail._DocumentType === '0001' &&
                orderDetail._DraftOrderFlag === 'N' &&
                orderDetail.OrderLines.OrderLine.length > 0
                )
            {
                //check all orderline status are Scheduled, ignore Cancelled line if all other lines are in scheduled
                var isAllLinesScheduledOrCanceledStatus = true;
                var isOneLineInScheduled = false;
                var scheduledRegEx = /^\s*Scheduled\s*$/i;
                var scheduledOrCancelledRegEx = /^\s*(Scheduled|Cancelled)\s*$/i;

                for (var i = 0; i < orderDetail.OrderLines.OrderLine.length; i++) {
                    if (scheduledRegEx.test(orderDetail.OrderLines.OrderLine[i]._Status)) {
                        isOneLineInScheduled = true;
                    }

                    if (!scheduledOrCancelledRegEx.test(orderDetail.OrderLines.OrderLine[i]._Status)) {
                        isAllLinesScheduledOrCanceledStatus = false;
                    }
                }

                if (isOneLineInScheduled && isAllLinesScheduledOrCanceledStatus) {
                    isCanCancel = true;
                }
            }
            //$securityService.canCancelOrder();
        }
