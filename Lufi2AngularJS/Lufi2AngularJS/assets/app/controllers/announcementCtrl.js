angular.module('manageAnnouncement', ['ui.bootstrap'])
    .controller('announcementCtrl', ['$scope', 'announcements', '$location', '$filter', '$modal', 'numberOnlyKeyPressValidator', function ($scope, $announcements, $location, $filter, $modal, $numberOnlyKeyPressValidator) {
        ...
        $scope.maxDate = new Date("9999-01-01");
        ...
        var success4 = function (data4) {
            ...
            $scope.header = "";
            $scope.detail = "";
            $scope.expDate = $filter('date')(new Date(), "MM-dd-yyyy");
        ...
        var validated = function () {
            ...
            if (!angular.isDefined($scope.expDate) || isNaN($scope.expDate) || !angular.isDate($scope.expDate)) {
                valid = false;
                swal({ title: "Invalid Expiration Date", text: "Expiration Date is not valid.", showConfirmButton: true });
            }
            else {
                var enteredDate = $filter('date')(new Date($scope.expDate), 'yyyy-MM-dd');
                var todaysDate = $filter('date')(new Date(), 'yyyy-MM-dd');

                if (todaysDate > enteredDate) {
                    valid = false;
                    swal({ title: "Invalid Expiration Date", text: "Expiration date must be equal or greater than today's date.", showConfirmButton: true });
                }
            }

            //check that a store is selected
            if (!angular.isArray($scope.displayFor) || $scope.displayFor.length < 1) {
                valid = false;
                swal({
                    title: "Invalid Store", text: 'Please select at least 1 store or "All Stores"', showConfirmButton: true
                    });
            }
            return valid;
        }
        ...
        $scope.save = function () {
            if (validated()) {
                ...
                if ($scope.displayFor[0]== "All Stores") {
                    storeList.push("");
                }
                else {
                    storeList = $scope.displayFor;
                }

                $announcements.addException(createExceptionRequest, storeList, success4, error4);
            }
        }
