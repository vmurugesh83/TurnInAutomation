...
appServices.factory( 'announcements', ['$http', '$cacheFactory', '$filter', 'sendSMTPErrorEmail', '$q', function ( $http, $cacheFactory, $filter, $sendSMTPErrorEmail, $q )
{
    ...
    var retrieve = function ( param, caller, success, error )
    {
        ...
        {
            //fix broker SOAP to JSON issue that results when only 1 object is returned.
            if (angular.isDefined(response.GetExceptionListRes.return) && !angular.isArray(response.GetExceptionListRes.return) && angular.isDefined(response.GetExceptionListRes.return.Inbox)) {
                var singleObject = response.GetExceptionListRes.return.Inbox;

                response.GetExceptionListRes.return = [];
                response.GetExceptionListRes.return.push(singleObject);
            }
            ...
            for ( var i = 0; i < response.GetExceptionListRes.return.length; i++ )
            {
                ...
                var tempDate = new Date(response.GetExceptionListRes.return[i]._GeneratedOn)
                ...
                var tempType = response.GetExceptionListRes.return[i]._ExceptionType;
                var tempOrder = response.GetExceptionListRes.return[i]._OrderNo;
                ...
                var resolutionDate = new Date(response.GetExceptionListRes.return[i]._ResolutionDate)
               ...
               //resolutionDate = ( caller === "MANAGER" ) ? $filter( 'date' )( new Date( resolutionDate ), 'yyyy-MM-dd' ) : $filter( 'date' )( new Date( resolutionDate.replace( /-/g, '\/' ).replace( /T.+/, '' ) ), 'yyyy-MM-dd' );
                // old resolutionDate = $filter( 'date' )( new Date( resolutionDate.replace( /-/g, '\/' ).replace( /T.+/, '' ) ), 'yyyy-MM-dd');
             
                if (tempHeader == "" && (tempType == "PICKUP" || tempType == "RESTOCK"))
                {
                    if (tempType == "PICKUP") {
                        tempHeader = "Order " + tempOrder + " - Open"

                    } else if (tempType == "RESTOCK") {
                        tempHeader = "Order " + tempOrder + " - Restock"
                    }
                    
                }

                if (tempLocations == ""){
                    tempLocations = "All Stores";
                }

                announcement = { key: tempKey, date: tempDate, header: tempHeader, detail: tempDetail, expDate: tempExpDate, locations: tempLocations, resolutionDate: resolutionDate, exceptionType: tempType};

                localArray.push( announcement );

                announcement = { key: "", date: "", header: "", detail: "", expDate: "", locations: "", resolutionDate:"", exceptionType:""};
