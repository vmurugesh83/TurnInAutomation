...
Public Class NoSampleReportDAO
    ...
    Public Function GetNoSampleReportInfo(ByVal POStartShipDate As Date) As IList(Of NoSampleReportInfo)
        Dim reportWeekly1Infos As New List(Of NoSampleReportInfo)
        Dim reportWeekly1Info As NoSampleReportInfo
        Dim sql As String = _spSchema + ".TU1161SP"
        ...
        Try
            Using rdr As DB2DataReader = ExecuteReader(ConnectionStringLocalTransaction, CommandType.StoredProcedure, sql, parms)
                While (rdr.Read())
                    reportWeekly1Info = New NoSampleReportInfo()
                    With reportWeekly1Info
                        ...
                        If Common.HasColumn(rdr, "SAMPLE_MERCH_ID") Then
                            .TurnInMerchId = CInt(rdr("SAMPLE_MERCH_ID"))
                        End If

                        If Common.HasColumn(rdr, "SAMPLE_STATUS_DESC") Then
                            .TurnInMerchStatus = CStr(rdr("SAMPLE_STATUS_DESC"))
                        End If

                        If Common.HasColumn(rdr, "SAMPLE_APVL_FLG") Then
                            .SampleApprovalFlag = CStr(rdr("SAMPLE_APVL_FLG"))
                        End If
                        ...
                        If Common.HasColumn(rdr, "VENDOR_IMAGE") Then
                            .IsVendorImage = CBool(rdr("VENDOR_IMAGE"))
                        End If
