<%@ Control Language="vb" AutoEventWireup="false" CodeBehind="eCommPreTurnInSetUpCtrl.ascx.vb"
    Inherits="TurnInProcessAutomation.PreTurnInSetUpCtrl" %>
        ...
        <telerik:RadSplitter runat="server" ID="splM" OnClientLoad="spl_Loaded" SkinID="BlueBT">
            <telerik:RadPane runat="server" ID="pNav" SkinID="pLNav" Scrolling="None">
                <div style="float: left; clear: none;" class="tabsContainer">
                    <telerik:RadSlidingZone ID="slZone" runat="server" OnClientLoaded="OnClientLoaded" SlideDirection="Right" ClickToOpen="false">
                                ...
                                <div>
                                    ...
                                    <span class="CheckBox" style="padding-left: 5px; padding-right: 5px;">
                                        <asp:CheckBox ID="chkNotAvailable" name="chkNotAvailable" runat="server"/><label
                                            for="chkNotAvailable">Sample Not Available</label></span><span class="Label"> </span>
                                    <br />
                                    <br />
                                    <span class="Label" style="font-weight: bold; padding-left: 5px;">Web Activity</span>
                                    <br />
                                    <span class="CheckBox" style="padding-left: 5px; padding-right: 5px;">
                                        <asp:CheckBox ID="chkActiveOnWeb" name="chkActiveOnWeb" runat="server"/><label for="chkActiveOnWeb">Active on Web</label>
                                    </span>
                                    <br />
                                    <span class="CheckBox" style="padding-left: 5px; padding-right: 5px;">
                                        <asp:CheckBox ID="chkNotActiveOnWeb" name="chkNotActiveOnWeb" runat="server"/><label for="chkNotActiveOnWeb">Not Active on Web</label></span><span class="Label"> </span>
                                    <br />
                                    <br />
                                    <span class="Label" style="font-weight: bold; padding-left: 5px;">Turnin Type</span>
                                    <br />
                                    <span class="CheckBox" style="padding-left: 5px; padding-right: 5px;">
                                        <asp:CheckBox ID="chkTurnInType" name="chkTurnInType" runat="server"/><label for="chkTurnInType">Not in Turn-in</label>
                                    </span>
                                    <br />
                                </div>
                            </asp:PlaceHolder>
            ...
            <telerik:RadPane ID="pMainNav" runat="server" Scrolling="None">
                <telerik:RadPanelBar runat="server" ID="rpbResultsTab" Height="470px" Width="100%"
                    ExpandMode="FullExpandedItem" OnClientItemClicked="OnClientClicked">
                    <Items>
                                    ...
                                    <ItemTemplate>
                                        <table id="tblPreTurnInSetUpCtrl" runat="server" cellpadding="0" style="width: 100%;">
                                            ...
                                            <tr>
                                                ...
                                                <td width="85%">
                                                    <telerik:RadDatePicker ID="dpStartShipDate" Style="vertical-align: middle; width: 120px;" runat="server"
                                                        TabIndex="4" UseEmbeddedScripts="false" CssClass="dpWidth">
                                    ...
                                    <ItemTemplate>
                                        <table id="tblPreTurnInSetUpCtrl" runat="server" cellpadding="0" style="width: 100%;">
                                            ...
                                            <tr>
                                                ...
                                                <td width="85%" nowrap="nowrap" valign="middle">
                                                    <telerik:RadComboBox ID="cmbVendorStyle" runat="server" OnItemsRequested="cmbVendorStyle_ItemsRequested"
                                                        class="RadComboBox_Vista" AllowCustomText="false" AppendDataBoundItems="True"
                                                        OnClientBlur="OnClientBlurHandler" Width="130px" TabIndex="5" DropDownWidth="205px"
                                                        Enabled="false" OnClientSelectedIndexChanged="OnClientSelectedIndexChanged">
                                                        <CollapseAnimation Type="OutQuint" Duration="200"></CollapseAnimation>
                                                    </telerik:RadComboBox>
                                                    <asp:ImageButton ID="imgAddVendorStyle" runat="server" ImageUrl="~/Images/Add.gif"
                                            ...
                                            <tr>
                                                ...
                                                <td width="85%">
                                                    <telerik:RadDatePicker ID="dpCreatedSince" Style="vertical-align: middle; width: 100px;" runat="server"
                                                        TabIndex="4" UseEmbeddedScripts="false" CssClass="dpWidth">
                                                        <DateInput ID="diCreatedSince" TabIndex="4" DisplayDateFormat="MM/dd/yyyy" runat="server"></DateInput><Calendar ID="calCreatedSince" UseRowHeadersAsSelectors="False" UseColumnHeadersAsSelectors="False"
                                                            ViewSelectorText="x" runat="server">
                                                        </Calendar>
        ...
        <telerik:RadCodeBlock runat="server" ID="rcb1">
            <script type="text/javascript">
                ...
                function OnClientLoaded(sender, args) {
                    ...
                    var resultsTab = document.getElementById('<%=rpbResultsTab.ClientID %>');
                    if (resultsTab != null) {
                        OnClientClicked(null, null);
                    }
                }
                function OnClientClicked(sender, args) {
                    var resultsTab = $find('<%= rpbResultsTab.ClientID%>');
                    var selectedText = "";
                    if (resultsTab.get_selectedItem() != null)
                    {
                        selectedText = resultsTab.get_selectedItem().get_text();
                    }
                    if ((sender == null && selectedText.indexOf('Filter by ISN') <= -1 && selectedText.indexOf('Filter by Hierarchy') <= -1) || (args != null && args.get_item().get_text().indexOf('Ship Date') > -1)) {
                        document.getElementById('<%=chkAvailableAndApproved.ClientID%>').checked = true;
                        if (sender != null) {
                            document.getElementById('<%=chkNotAvailable.ClientID%>').checked = false;
                        }
                        document.getElementById('<%=chkActiveOnWeb.ClientID%>').checked = false;
                        document.getElementById('<%=chkActiveOnWeb.ClientID%>').disabled = true;
                        document.getElementById('<%=chkNotActiveOnWeb.ClientID%>').checked = true;
                        document.getElementById('<%=chkNotActiveOnWeb.ClientID%>').disabled = true;
                        document.getElementById('<%=chkTurnInType.ClientID%>').checked = true;
                        document.getElementById('<%=chkTurnInType.ClientID%>').disabled = true;

                    }
                    else
                    {
                        document.getElementById('<%=chkAvailableAndApproved.ClientID%>').disabled = false;
                        document.getElementById('<%=chkAvailableAndApproved.ClientID%>').checked = false;
                        document.getElementById('<%=chkNotAvailable.ClientID%>').disabled = false;
                        document.getElementById('<%=chkNotAvailable.ClientID%>').checked = false;
                        document.getElementById('<%=chkActiveOnWeb.ClientID%>').disabled = false;
                        document.getElementById('<%=chkNotActiveOnWeb.ClientID%>').disabled = false;
                        document.getElementById('<%=chkNotActiveOnWeb.ClientID%>').checked = false;
                        document.getElementById('<%=chkTurnInType.ClientID%>').disabled = false;
                        document.getElementById('<%=chkTurnInType.ClientID%>').checked = false;
                    }
                }
           </script>
