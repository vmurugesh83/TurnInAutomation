...
Public Class PreTurnInSetUpCtrl
...
#Region "Properties"
    ...
    Public Property SelectedVendorStyles As List(Of String)
        Get
            If Session("PreTurnInSetUpCtrl.SelectedVendorStyles") Is Nothing Then
                Session("PreTurnInSetUpCtrl.SelectedVendorStyles") = New List(Of String)
            End If
            Return CType(Session("PreTurnInSetUpCtrl.SelectedVendorStyles"), List(Of String))
        End Get
        Set(value As List(Of String))
            Session("PreTurnInSetUpCtrl.SelectedVendorStyles") = value
        End Set
    End Property
...
#End Region
    ...
    Private Sub Page_Load(sender As Object, e As System.EventArgs) Handles Me.Load
        If Not Page.IsPostBack Then
            ...
            BindrcbAds()
            dpStartShipDate.SelectedDate = Date.Now().AddDays(CDbl(ConfigurationManager.AppSettings("CFGDefaultStartShipDays")))
        End If
    End Sub
...
#End Region
    ...
    Protected Sub rpbResultsTab_ItemClick(sender As Object, e As RadPanelBarEventArgs)
        If rpbResultsTab.Items(0).Expanded Then
            slZone.Enabled = False
            slZone.ClickToOpen = False
            chkActiveOnWeb.Checked = False
            chkAvailableAndApproved.Checked = False
            chkNotActiveOnWeb.Checked = False
            chkNotAvailable.Checked = False
            chkTurnInType.Checked = False
        Else
            slZone.Enabled = True
            slZone.ClickToOpen = True
        End If
    End Sub
    ''' <summary>
    ...
    Public Sub SetDefaultShipDays()
        Dim defaultShipDays As Integer = CInt(ConfigurationManager.AppSettings("CFGDefaultStartShipDays"))
        Dim customizedShipDays As Integer = 0
        Dim vendorStyles As New List(Of String)

        For Each vendorStyle As ListItem In lbPOVendorStyles.Items
            vendorStyles.Add(CType(vendorStyle, ListItem).Text.Trim())
        Next

        If cmbPOVendorStyle.SelectedValue <> String.Empty AndAlso Not vendorStyles.Contains(cmbPOVendorStyle.SelectedValue.Trim()) Then
            vendorStyles.Add(cmbPOVendorStyle.SelectedValue)
        End If
