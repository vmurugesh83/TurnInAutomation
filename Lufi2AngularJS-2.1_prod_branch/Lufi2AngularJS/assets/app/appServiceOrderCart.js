...
.factory('orderCart', ["$log", "btProp", 'POSService', '$http', '$rootScope', 'errorObj', 'giftRegistryService', 'serviceArrayFix', 'repriceService', '$q', '$state', 'customer', 'isPoBoxAddress', 'bigTicketValidate', 'itemProperty', 'loggerService', 'sendSMTPErrorEmail',
     function ($log, btProp, POSService, $http, $rootScope, errorObj, giftRegistryService, serviceArrayFix, repriceService, $q, $state, $customer, $isPoBoxAddress, $bigTicketValidate, $itemProperty, $loggerService, $sendSMTPErrorEmail) {
         ...
         var _setStoreAndAssociateId = function (storeNumber, associateId) {
             ...
             if (angular.isDefined(storeNumber) && storeNumber.length > 0) {
                 ...
                 var contract = {
                     ItemLocateInput: {
                         input: {
                             _OrganizationKey: storeNumber
                         }
                     }
                 }
                 $http.post(url, angular.toJson(contract)).success(function (data) {
                     if (data.getOrganizationListResponse.Organization.Extn && data.getOrganizationListResponse.Organization.Extn._ExtnEntryType) {
                         orderCart.Extn._EntryType = data.getOrganizationListResponse.Organization.Extn._ExtnEntryType;
                         orderCart._EntryType = data.getOrganizationListResponse.Organization.Extn._ExtnEntryType;
                     } else {
             ...
             for (var registryValue in missingRegistryAddress) {
                 ...
                 promiseArr.push(giftRegistryService.preferredAddressPromise(registryValue).then(function (response) {
                     if (response.data.PreferredAddressOutput.PreferredAddressResponse.IsValid === 'true' &&
                         ...
                         //TODO: Can I throw inside of a promise?
                         var ErrorMessage = '';
         ...
         var _copyAddress = function (source, destination) {
             ...
             if (source) {
                 ...
                 destination._DayPhone = source._DayPhone ? source._DayPhone : "";
                 destination._EveningPhone = source._EveningPhone ? source._EveningPhone : "";
         ...
         var _setGiftRegistry = function (arrayOfPrimeSubLineObjects, regNo, regAddress) {
             ...
             if (regNo && regAddress) {
                 ...
                 if (!(regAddress.PersonInfo && angular.isString(regAddress.PersonInfo._DayPhone) && (regAddress.PersonInfo._DayPhone.length === 10))) {
                     ...
                     regAddress.PersonInfo._DayPhone = orderCart.btLogic.defaultBillToAddress.PersonInfo._DayPhone.toString();
         ...
         var _constructOrderLine = function (itemObject, quantity) {
             ...
             //TODO: get broker URL from properties Service
             if (angular.isDefined(copyItemObject.imageid) && (copyItemObject.imageid.toString().trim() !== "")) {
