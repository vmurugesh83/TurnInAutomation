    ...
    .controller('itemLocateCtrl', ['$scope', 'itemToLocate', 'itemLocate', 'POSService', '$location', function ($scope, $itemToLocate, $itemLocate, POSService, $location) {
        ...
        $scope.refreshIScroll = function () {
            ...
            $scope.getZip = function () {
                $itemLocate.getOrganizationList($scope.posParam.storeNumber).success(function (response) {
                    var Organization = response.getOrganizationListResponse.Organization;
                    $scope.itemLocateZip = Organization.CorporatePersonInfo._ZipCode;
                    $scope.SearchType();
                    $scope.search();
                }).error(function (data) {
        ...
        $scope.search = function () {
            ...
            if (($scope.itemLocateCity == "" || $scope.itemLocateState == "") && ($scope.itemLocateZip == "")) {
                ...
                $itemLocate.locate($scope.itemLocateZip, $scope.itemLocateCity, $scope.itemLocateRadius, $scope.itemLocateState, $scope.currentItem.sku, $scope.posParam.storeNumber).success(function (response) {
                    $scope.NodeList = response.GetSurroundingNodeList.NodeList;
                    $scope.noNodesToDisplay = false;
                    if ($scope.NodeList.Node == undefined) {
                        $scope.noNodesToDisplay = true;
                    }
                    else if ($scope.NodeList.Node.length == undefined) {             
                        singleNode = [];
                        singleNode[0] = $scope.NodeList.Node;
                        $scope.NodeList.Node = singleNode;
                    };
