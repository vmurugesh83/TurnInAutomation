...
.controller('itemDetailCtrl', ['$scope', '$state', '$modal', 'itemSearch', 'itemDetail', 'orderCart', 'numberOnlyKeyPressValidator', 'bigTicketValidate', 'itemToLocate', 'loggerService', 'btProp',
    function ($scope, $state, $modal, $itemSearch, $itemDetail, $orderCart, $numberOnlyKeyPressValidator, $bigTicketValidate, $itemToLocate, $loggerService, $btProp) {
    ...
    $scope.disableCartButton = function (item) { 
        ...
        if (item == undefined || item.itemDetail == undefined || (item.itemDetail === null) ||
            ...
            var qty = $scope.txtQtyInput
            if (parseInt(item._AvailableQty) < qty || parseInt(item._AvailableQty) == '0' || item.isgwp == "Y" ||
...
.controller('itemDescriptionCtrl', ['$scope', '$modalInstance', 'currentItem', 'isCart','$state','orderCart', '$modal',
				function ($scope, $modalInstance, currentItem, isCart, $state, $orderCart, $modal) {
